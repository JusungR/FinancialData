...
python src/collect_kr.py --days 7      # 최근 7일 중 누락분만 수집 (멱등)
...
1. **DART 키**: [opendart.fss.or.kr](https://opendart.fss.or.kr) 가입 → 인증키 발급 → repo Settings → Secrets and variables → Actions → `DART_API_KEY` 등록
2. **ECOS 키**: [ecos.bok.or.kr](https://ecos.bok.or.kr) → OpenAPI 인증키 → `ECOS_API_KEY` 등록
3. **KOSIS 키**: [kosis.kr](https://kosis.kr) 공유서비스 → OpenAPI → `KOSIS_API_KEY` 등록
4. Settings → Actions → General → Workflow permissions = **Read and write**
5. Actions 탭에서 `smoke-test` 수동 실행 → 전부 통과 확인 (KRX 해외 IP 차단 여부 검증)
6. `config/` watchlist·지표를 원하는 목록으로 편집
